...
public class OAuth2AuthenticationMethodType extends AuthenticationMethodType {
    ...
    public void exportData(Configuration config, AuthenticationMethod authMethod) {
        ...
        method.extraTokenParams.forEach(
                (k, v) ->
                        list.add(
                                encoder.encodeToString(k.getBytes(StandardCharsets.UTF_8))
                                        + ":"
                                        + (v == null
                                                ? ""
                                                : encoder.encodeToString(
                                                        v.getBytes(StandardCharsets.UTF_8)))));
        config.setProperty(CONTEXT_CONFIG_AUTH_OAUTH2_EXTRA_PARAM, list);
    }
    ...
    public void importData(Configuration config, AuthenticationMethod authMethod)
        ...
        try {
            ...
            for (Object entry : config.getList(CONTEXT_CONFIG_AUTH_OAUTH2_EXTRA_PARAM)) {
            ...
            method.setExtraTokenParams(extraParams);
        } catch (Exception e) {
            throw new ConfigurationException(e);
        }
    }
