...
class DomXssScanRuleUnitTest extends ActiveScannerTestUtils<DomXssScanRule> {
    ...
    void tearDown() {
    	...
    	parent = null; // TODO makes no difference - parent stays the same
    }
    ...
    @ParameterizedTest
    @MethodSource("testBrowsers")
    void shouldNotRequestReferencedUrlExcludedGlobally(String browser) throws NullPointerException, IOException {
    	// Given
    	// TODO
...
/* */
    @ParameterizedTest
    @MethodSource("testBrowsers")
    void shouldNotRequestReferencedUrlExcludedInSession(String browser) throws NullPointerException, IOException {
    	// Given TODO
        Context context = mock(Context.class);
        given(context.isExcluded(ArgumentMatchers.anyString())).willReturn(false);
        given(context.isExcluded(ArgumentMatchers.matches(".*/test.png"))).willReturn(true); // TODO!!
        ...
        this.nano.addHandler(
                new NanoServerHandler(test) {
                    ...
                    protected NanoHTTPD.Response serve(NanoHTTPD.IHTTPSession session) {
                    	...
                    	if ("/test.png".equals(session.getUri())) {
                    		System.out.println("SBSB PNG accessed! " + session.getUri());
                    		accessedImage.set(true);
                    	}
                        return newFixedLengthResponse(
                                "<html><body>\n"
        ...
        // When
		System.out.println("SBSB Test Start vvvvvvvvvvvvvv"); // TODO
		System.out.println("SBSB test parent is " + parent.hashCode()); // TODO
        this.rule.scan();
		System.out.println("SBSB Test End   ^^^^^^^^^^^^^^"); // TODO
