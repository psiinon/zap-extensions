...
public class McpParam extends VersionedAbstractParam {
    ...
    private static final String SECURE_ONLY_KEY = MCP_KEY + ".secureOnly";
    ...
    /** Generates a random alphanumeric key for accessing the server. */
    ...
    /** Returns whether the MCP server should only accept secure (HTTPS) connections. */
    public boolean isSecureOnly() {
        return secureOnly;
    }

    /** Sets whether the MCP server should only accept secure (HTTPS) connections. */
    public void setSecureOnly(boolean secureOnly) {
        this.secureOnly = secureOnly;
        getConfig().setProperty(SECURE_ONLY_KEY, secureOnly);
    }
}
