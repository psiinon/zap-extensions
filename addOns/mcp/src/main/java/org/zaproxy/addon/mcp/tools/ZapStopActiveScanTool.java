...
/** MCP tool that stops the active scan plan if it is running. */
public class ZapStopActiveScanTool implements McpTool {

    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    @Override
    public String getName() {
        return "zap_stop_active_scan";
    }

    @Override
    public String getDescription() {
        return Constant.messages.getString("mcp.tool.stopactivescan.desc");
    }
    ...
    public ObjectNode getInputSchema() {
        ...
        properties
                ...
                .put(
                        "description",
                        Constant.messages.getString("mcp.tool.stopactivescan.param.scanid"));
    ...
    public McpToolResult execute(JsonNode arguments) throws McpToolException {
        ...
        if (scanIdNode == null || scanIdNode.isNull() || !scanIdNode.isTextual()) {
            throw new McpToolException(
                    Constant.messages.getString("mcp.tool.stopactivescan.error.missingscanid"));
        }
        ...
        if (extAutomation.getProgress(scanId) < 0) {
            throw new McpToolException(
                    Constant.messages.getString(
                            "mcp.tool.stopactivescan.error.scanidnotfound", scanId));
        }

        extAutomation.stopLongRunningJob(scanId);

        return McpToolResult.success(
                Constant.messages.getString("mcp.tool.stopactivescan.success"));
    }
}
