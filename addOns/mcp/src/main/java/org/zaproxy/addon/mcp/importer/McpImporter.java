...
public class McpImporter {
    ...
    public ImportResults importServer(ImportConfig config) {
        ...
        URI serverUri;
        try {
            serverUri = new URI(config.serverUrl(), false);
     ...
     * 6570 template placeholder (e.g. {@code "{target}"}) so that {@code VariantJsonRpc} exposes
     ...
     * template placeholder for each argument name so that {@code VariantJsonRpc} exposes them as
     * fuzz parameters.
     */
