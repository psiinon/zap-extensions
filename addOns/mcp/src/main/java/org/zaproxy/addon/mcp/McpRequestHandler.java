...
class McpRequestHandler {
        ...
        try {
            ...
            switch (method) {
                ...
                case "prompts/list":
                    ...
                    break;
                case "prompts/get":
                    result = handlePromptsGet(requestObj, id);
                    break;
                default:
                    result =
                            errorResponse(
    ...
    private String handleResourcesRead(ObjectNode request, JsonNode id) {
        ...
        try {
            ...
            contentHolder.append(resource.readContent(uri));
    ...
    private String handlePromptsGet(ObjectNode request, JsonNode id) {
        JsonNode params = request.get("params");
        if (params == null || !params.isObject()) {
            return errorResponse(id, ERROR_INVALID_PARAMS, "Invalid params");
        }
        ...
        String name = params.has("name") ? params.get("name").asText() : null;
        if (name == null || name.isEmpty()) {
            return errorResponse(id, ERROR_INVALID_PARAMS, "Prompt name not specified");
        }

        McpPrompt prompt = promptRegistry.getPrompt(name);
        if (prompt == null) {
            return errorResponse(id, ERROR_INVALID_PARAMS, "Unknown prompt: " + name);
        }

        Map<String, String> arguments = new HashMap<>();
        JsonNode argsNode = params.get("arguments");
        if (argsNode != null && argsNode.isObject()) {
            argsNode.properties().forEach(e -> arguments.put(e.getKey(), e.getValue().asText()));
        }

        ArrayNode messagesArray = OBJECT_MAPPER.createArrayNode();
        for (McpPrompt.PromptMessage message : prompt.getMessages(arguments)) {
            ...
            contentNode.put("text", message.text());
    ...
    private static ObjectNode toSchemaNode(InputSchema schema) {
        ...
        schema.properties().forEach((name, def) -> propsNode.set(name, toPropertyNode(def)));
        ArrayNode requiredNode = schemaNode.putArray("required");
        schema.required().forEach(requiredNode::add);
        return schemaNode;
    }

    private static ObjectNode toPropertyNode(PropertyDef def) {
        ObjectNode node = OBJECT_MAPPER.createObjectNode();
        node.put("type", def.type());
        if (def.description() != null) {
            node.put("description", def.description());
        }
        if (def.items() != null) {
            node.set("items", toPropertyNode(def.items()));
        }
        return node;
    }
