...
public class ZapStartSpiderTool implements McpTool {
    ...
    public ObjectNode getInputSchema() {
        ...
        properties
                ...
                .put(
                        "description",
                        Constant.messages.getString("mcp.tool.startspider.param.target"));
    ...
    public McpToolResult execute(JsonNode arguments) throws McpToolException {
        ...
        if (targetNode == null || targetNode.isNull() || !targetNode.isTextual()) {
            throw new McpToolException(
                    Constant.messages.getString("mcp.tool.startspider.error.missingtarget"));
        }

        String target = targetNode.asText().trim();
        if (target.isEmpty()) {
            throw new McpToolException(
                    Constant.messages.getString("mcp.tool.startspider.error.emptytarget"));
        }
        ...
        try {
            ...
            AutomationJob spiderJob = extAutomation.getAutomationJob("spider");
            if (spiderJob == null) {
                throw new RuntimeException(
                        new McpToolException(
                                Constant.messages.getString(
                                        "mcp.tool.startspider.error.nospider")));
            }
            spiderJob = spiderJob.newJob();
            ...
            if (isUrl(targetFinal)) {
                try {
                    ...
                    throw new RuntimeException(
                            new McpToolException(
                                    Constant.messages.getString(
                                            "mcp.tool.startspider.error.invalidurl", targetFinal),
                                    e));
                }
                ...
                if (context == null) {
                    throw new RuntimeException(
                            new McpToolException(
                                    Constant.messages.getString(
                                            "mcp.tool.startspider.error.contextnotfound",
                                            targetFinal)));
                }
                plan.getEnv().addContext(context);
            }
            plan.addJob(spiderJob);

            extAutomation.registerPlan(plan);
            extAutomation.runPlanAsync(plan);

            scanId = waitForSpiderId((LongRunningJob) spiderJob, spiderJob);
        } catch (Exception e) {
            ...
            throw new McpToolException(
                    Constant.messages.getString(
                            "mcp.tool.startspider.error.failed", e.getMessage()),
                    e);
        }

        return McpToolResult.success(
                Constant.messages.getString("mcp.tool.startspider.success", scanId));
    }

    private static String waitForSpiderId(LongRunningJob longRunningJob, AutomationJob job) {
        int maxAttempts = 50;
        int sleepMs = 200;
        for (int i = 0; i < maxAttempts; i++) {
            ...
            if (job.getStatus() == AutomationJob.Status.COMPLETED) {
                throw new RuntimeException(
                        new McpToolException(
                                Constant.messages.getString(
                                        "mcp.tool.startspider.error.failed",
                                        "spider job finished without starting")));
            }
        }
        throw new RuntimeException(
                new McpToolException(
                        Constant.messages.getString(
                                "mcp.tool.startspider.error.failed", "timeout")));
    }
