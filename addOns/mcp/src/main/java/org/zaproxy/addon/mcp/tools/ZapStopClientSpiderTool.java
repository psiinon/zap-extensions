...
/** MCP tool that stops the Client Spider plan if it is running. */
public class ZapStopClientSpiderTool implements McpTool {

    @Override
    public String getName() {
        return "zap_stop_client_spider";
    }

    @Override
    public String getDescription() {
        return Constant.messages.getString("mcp.tool.stopclientspider.desc");
    }
                ...
                Map.of(
                        ...
                        InputSchema.PropertyDef.ofString(
                                Constant.messages.getString(
                                        "mcp.tool.stopclientspider.param.scanid"))),
                List.of("scan_id"));
    }
    ...
    public McpToolResult execute(ToolArguments arguments) throws McpToolException {
        ...
        if (scanId == null || scanId.isBlank()) {
            throw new McpToolException(
                    Constant.messages.getString("mcp.tool.stopclientspider.error.missingscanid"));
        }
        scanId = scanId.trim();
        ...
        if (extAutomation.getLongRunningJobProgress(scanId) < 0) {
            throw new McpToolException(
                    Constant.messages.getString(
                            "mcp.tool.stopclientspider.error.scanidnotfound", scanId));
        }

        extAutomation.stopLongRunningJob(scanId);

        return McpToolResult.success(
                Constant.messages.getString("mcp.tool.stopclientspider.success"));
    }
}
