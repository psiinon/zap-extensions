...
public class HistoryEntryResource implements McpResource {
    ...
    public String readContent(String uri) {
        ...
        try {
            ...
            RecordHistory rec = tableHistory.read(id);
            if (rec == null) {
                return errorJson(
                        Constant.messages.getString(
                                "mcp.resource.historyentry.error.notfound", id));
            }
            HttpMessage msg = rec.getHttpMessage();
            ObjectNode result = OBJECT_MAPPER.createObjectNode();
            result.put(
                    "requestHeader",
                    msg.getRequestHeader() != null ? msg.getRequestHeader().toString() : "");
            result.put(
                    "requestBody",
                    msg.getRequestBody() != null ? msg.getRequestBody().toString() : "");
            result.put(
                    "responseHeader",
                    msg.getResponseHeader() != null ? msg.getResponseHeader().toString() : "");
            result.put(
                    "responseBody",
