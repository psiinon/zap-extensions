...
class McpParamUnitTest {
    ...
    void shouldGenerateRandomKeyWithCorrectLength() {
        String key = McpParam.generateRandomKey();

        assertThat(key.length(), is(equalTo(32)));
        assertThat(key, matchesRegex("[A-Za-z0-9]{32}"));
    }

    @Test
    void shouldGenerateDifferentKeysEachTime() {
        String key1 = McpParam.generateRandomKey();
        String key2 = McpParam.generateRandomKey();
    ...
    @Test
    void shouldLoadSecureOnlyFromConfig() {
        config.setProperty("mcp.secureOnly", false);
        param.load(config);

        assertThat(param.isSecureOnly(), is(false));
    }

    @Test
    void shouldSaveSecureOnlyToConfig() {
        param.setSecureOnly(false);

        assertThat(config.getBoolean("mcp.secureOnly", true), is(false));
    }
}
