...
/** Unit tests for {@link ZapStartSpiderTool} mode checks. */
class ZapStartSpiderToolUnitTest {

    private ExtensionLoader extensionLoader;
    private Model model;
    private Session session;
    private ExtensionAutomation extAutomation;
    private ZapStartSpiderTool tool;
    ...
    void setUp() throws Exception {
        ...
        given(extAutomation.getAutomationJob("spider")).willReturn(jobTemplate);
        given(jobTemplate.newJob()).willReturn(job);
        Model.setSingletonForTesting(model);
        Control.initSingletonForTesting(model, extensionLoader);
        Control.getSingleton().setMode(Mode.standard);
        tool = new ZapStartSpiderTool();
    }

    @Test
    void shouldReturnCorrectName() {
        assertThat(tool.getName(), equalTo("zap_start_spider"));
    }
    ...
    void shouldRejectInvalidIntegerParam() {
        ...
        ToolArguments args =
                new ToolArguments(
                        Map.of("target", "https://example.com/", "max_crawl_depth", "abc"),
                        Map.of());
