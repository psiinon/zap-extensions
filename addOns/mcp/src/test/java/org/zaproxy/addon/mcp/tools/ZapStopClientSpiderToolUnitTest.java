...
/** Unit tests for {@link ZapStopClientSpiderTool}. */
class ZapStopClientSpiderToolUnitTest {

    private ExtensionLoader extensionLoader;
    private ExtensionAutomation extAutomation;
    private ZapStopClientSpiderTool tool;
    ...
    void setUp() {
        ...
        tool = new ZapStopClientSpiderTool();
    }

    @Test
    void shouldReturnCorrectNameAndInputSchema() {
        assertThat(tool.getName(), equalTo("zap_stop_client_spider"));
    ...
    void shouldStopScanSuccessfully() throws McpToolException {
        given(extAutomation.getLongRunningJobProgress("clientspider-1")).willReturn(50);
        ToolArguments args = new ToolArguments(Map.of("scan_id", "clientspider-1"), Map.of());

        McpToolResult result = tool.execute(args);

        verify(extAutomation).stopLongRunningJob("clientspider-1");
        assertThat(result.text(), containsString("!mcp.tool.stopclientspider.success!"));
    }
}
